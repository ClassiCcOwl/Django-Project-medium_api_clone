...
from core_apps.bookmarks.serializer import BookmarkSerializer
...
class TagListField(serializers.Field):
    ...
    def to_internal_value(self, data):
        if not isinstance(data, list):
            raise serializers.ValidationError("Excpected a list of tags")
        tag_object = []
        for tag_name in data:
            tag_name = tag_name.strip()
            if not tag_name:
                continue
            tag_object.append(tag_name)
        return tag_object
...
class ArticleSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        instance.banner_image = validated_data.get(
            "banner_inage", instance.banner_image
        )
...
class ClapSerializer(serializers.ModelSerializer):
    ...
    user_first_name = serializers.CharField(source="article.first_name", read_only=True)
